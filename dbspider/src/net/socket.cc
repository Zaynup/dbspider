...
namespace dbspider
{
    ...
    bool Socket::getOption(int level, int option, void *result, size_t *len)
    {
        ...
        if (rt)
        {
            DBSPIDER_LOG_DEBUG(g_logger) << "getOption sock=" << m_sock
    ...
    bool Socket::setOption(int level, int option, void *result, size_t len)
    {
        ...
        if (rt)
        {
            DBSPIDER_LOG_ERROR(g_logger) << "setOption sock=" << m_sock
                                         << " level=" << level << " option=" << option
                                         << " errno=" << errno << " errorstr=" << strerror(errno);
            return false;
        }
    ...
    void Socket::newSocket()
    {
        ...
        if (DBSPIDER_LIKELY(m_sock != -1))
        {
            initSocket();
        }
        else
        {
            DBSPIDER_LOG_ERROR(g_logger) << "socket(" << m_family << ", " << m_type << ", " << m_protocol << ") "
    ...
    Socket::ptr Socket::accept()
    {
        ...
        if (newsock < 0)
        {
            DBSPIDER_LOG_DEBUG(g_logger) << "accept(" << m_sock << ") "
                                         << "errno=" << errno << "errstr=" << strerror(errno);
            return nullptr;
        }
    ...
    bool Socket::bind(const Address::ptr address)
    {
        ...
        if (DBSPIDER_UNLIKELY(m_family != address->getFamily()))
        {
            DBSPIDER_LOG_ERROR(g_logger) << "bind sock.family(" << m_family << ") address.family(" << address->getFamily() << ")"
        ...
        {
            DBSPIDER_LOG_ERROR(g_logger) << "bind address=" << address->toString() << " error, errno="
    ...
    bool Socket::connect(const Address::ptr address, uint64_t timeout_ms)
    {
        ...
        if (DBSPIDER_UNLIKELY(m_family != address->getFamily()))
        {
            DBSPIDER_LOG_WARN(g_logger) << "connect sock.family(" << m_family << ") address.family(" << address->getFamily() << ")"
                                        << " not equal, address=" << address->toString();
            return false;
        }
        ...
        if (timeout_ms == (uint64_t)-1)
        {
            ...
            {
                DBSPIDER_LOG_WARN(g_logger) << "sock=" << m_sock << " connect(" << address->toString() << ") error, errno="
                                            << errno << " errstr=" << strerror(errno);
                close();
                return false;
            }
        ...
        else
        {
            ...
            {
                DBSPIDER_LOG_WARN(g_logger) << "sock=" << m_sock << " connect(" << address->toString() << ") error, timeout="
    ...
    bool Socket::listen(int backlog)
    {
        if (DBSPIDER_UNLIKELY(!isValid()))
        {
            DBSPIDER_LOG_ERROR(g_logger) << "listen error, sock=-1";
            return false;
        }
        if (::listen(m_sock, backlog))
        {
            DBSPIDER_LOG_ERROR(g_logger) << "listen error, errno=" << errno << " errstr=" << strerror(errno);
            return false;
        }
        return true;
    }
