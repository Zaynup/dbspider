...
namespace dbspider
{
                         ...
                         int family, int type, int protocol)
    {
        ...
        if (error)
        {
            DBSPIDER_LOG_DEBUG(g_logger) << "Address::Lookup getaddress(" << host << ", "
    ...
    bool Address::GetInterfaceAddresses(std::multimap<std::string, std::pair<Address::ptr, uint32_t>> &result, int family)
    {
        ...
        if (error)
        {
            DBSPIDER_LOG_DEBUG(g_logger) << "Address::GetInterfaceAddresses getifaddrs "
                                            " err="
        ...
        {
            DBSPIDER_LOG_ERROR(g_logger) << "Address::GetInterfaceAddresses exception";
            freeifaddrs(results);
            return false;
        }
        freeifaddrs(results);
    ...
    IPAddress::ptr IPAddress::Create(const char *host_name, uint16_t port)
    {
        ...
        if (error)
        {
            DBSPIDER_LOG_DEBUG(g_logger) << "IPAddress::Create(" << host_name
                                         << ", " << port << ") error=" << error
    ...
    IPv4Address::ptr IPv4Address::Create(const char *ip, uint16_t port)
    {
        ...
        if (result <= 0)
        {
            DBSPIDER_LOG_DEBUG(g_logger) << "IPv4Address::Create(" << ip << ", "
    ...
    IPv6Address::ptr IPv6Address::Create(const char *ip, uint16_t port)
    {
        ...
        if (result <= 0)
        {
            DBSPIDER_LOG_DEBUG(g_logger) << "IPv6Address::Create(" << ip << ", "
                                         << port << ") rt=" << result << " errno=" << errno
                                         << " errstr=" << strerror(errno);
            return nullptr;
        }
