...
namespace dbspider::rpc
{
    ...
    struct _RpcServerIniter
    {
        _RpcServerIniter()
        {
            ...
            g_heartbeat_timeout->addListener(
                [](const uint64_t &old_val, const uint64_t &new_val)
                {
                    DBSPIDER_LOG_INFO(g_logger) << "rpc server heartbeat timeout changed from "
                                                << old_val << " to " << new_val;
    ...
    void RpcServer::registerService(const std::string &name)
    {
        ...
        if (!rp)
        {
            DBSPIDER_LOG_WARN(g_logger) << "registerService:" << name << " fail, registry socket:" << m_registry->getSocket()->toString();
            return;
        }
        ...
        {
            DBSPIDER_LOG_WARN(g_logger) << result.toString();
        }
        DBSPIDER_LOG_INFO(g_logger) << result.toString();
    }
