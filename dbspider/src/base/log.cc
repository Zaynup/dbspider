...
namespace dbspider
{
    ...
    LogEvent::LogEvent(std::shared_ptr<Logger> logger, LogLevel::Level level, const char *file, int32_t line, int32_t threadId,
        ...
        : m_logger(logger), m_level(level), m_file(file), m_line(line), m_threadId(threadId), m_fiberId(fiberId), m_elapse(elapse), m_time(timestamp), m_threadName(threadName)
    {
    ...
    void LogFormatter::init()
    {
        ...
        static std::map<std::string, std::function<FormatItem::ptr(const std::string &)>> itemMap{
            {"m", [](const std::string &str)
             { return MessageFormatItem::ptr(new MessageFormatItem(str)); }},
            {"p", [](const std::string &str)
             { return LevelFormatItem::ptr(new LevelFormatItem(str)); }},
            {"r", [](const std::string &str)
             { return ElapseFormatItem::ptr(new ElapseFormatItem(str)); }},
            {"c", [](const std::string &str)
             { return LogNameFormatItem::ptr(new LogNameFormatItem(str)); }},
            {"t", [](const std::string &str)
             { return ThreadIdFormatItem::ptr(new ThreadIdFormatItem(str)); }},
            {"n", [](const std::string &str)
             { return NewLineFormatItem::ptr(new NewLineFormatItem(str)); }},
            {"d", [](const std::string &str)
             { return DateTimeFormatItem::ptr(new DateTimeFormatItem(str)); }},
            {"f", [](const std::string &str)
             { return FileNameFormatItem::ptr(new FileNameFormatItem(str)); }},
            {"l", [](const std::string &str)
             { return LineFormatItem::ptr(new LineFormatItem(str)); }},
            {"T", [](const std::string &str)
             { return TabLineFormatItem::ptr(new TabLineFormatItem(str)); }},
            {"F", [](const std::string &str)
             { return FiberIdFormatItem::ptr(new FiberIdFormatItem(str)); }},
            {"N", [](const std::string &str)
             { return ThreadNameFormatItem::ptr(new ThreadNameFormatItem(str)); }},
        };
    ...
    struct LogIniter
    {
        LogIniter()
        {
            ...
            for(auto &i : new_val){
                ...
                Logger::ptr logger;
                logger = DBSPIDER_LOG_NAME(i.name);
            ...
            for(auto &i : old_val){
                ...
                if(it == new_val.end()){
                    auto logger = DBSPIDER_LOG_NAME(it->name);
                    logger->clearAppender();
