...
namespace dbspider
{
                               ...
                               std::list<std::pair<std::string, YAML::Node>> &output)
    {
        ...
        {
            DBSPIDER_LOG_ERROR(DBSPIDER_LOG_ROOT()) << "config invalid name : " << perfix << " : " << node;
            return;
        }
    ...
    void Config::LoadFromYaml(const YAML::Node &root)
    {
        ...
        for (auto &item : allNodes)
        {
            ...
            if (key.empty())
                continue;
            // DBSPIDER_LOG_DEBUG(DBSPIDER_LOG_ROOT())<<key<<"="<<item.second<<std::endl;
    ...
    {
        // DBSPIDER_LOG_INFO(DBSPIDER_LOG_ROOT())<<"load file:"<<file;
