...
namespace dbspider
{
    ...
    public:
        ...
        std::string toString() override
        {
            ...
            {
                DBSPIDER_LOG_ERROR(DBSPIDER_LOG_ROOT()) << "ConfigVar::toString() exception "
        ...
        bool fromString(const std::string &str) override
        {
            ...
            {
                DBSPIDER_LOG_ERROR(DBSPIDER_LOG_ROOT()) << "ConfigVar::fromString() exception "
    ...
    public:
        ...
        static typename ConfigVar<T>::ptr Lookup(const std::string &name, const T &value, const std::string &description)
        {
            ...
            if (it != GetDatas().end())
            {
                ...
                if (tmp)
                {
                    DBSPIDER_LOG_INFO(DBSPIDER_LOG_ROOT()) << "lookup name=" << name << " already exist";
                    return tmp;
                }
                else
                {
                    DBSPIDER_LOG_ERROR(DBSPIDER_LOG_ROOT()) << "lookup name=" << name << " already exist but type not "
            ...
            {
                DBSPIDER_LOG_ERROR(DBSPIDER_LOG_ROOT()) << "lookup invalid name " << name;
